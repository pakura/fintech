...
<div class="page-title">
    <div class="title-env">
        <h1 class="title">
            <i class="{{$icon = icon_type('companies')}}"></i>
            Companies
        </h1>
        <p class="description">Edit companies</p>
    </div>
...
@endif
    ...
    <ul class="nav nav-tabs col-xs-4 right-aligned">
        <li>
            <a href="{{cms_route('files.index', ['companies', $current->id])}}">
...
<div class="panel panel-default">
    <div class="panel-heading">
        ...
        <a href="{{cms_route('companies.create', [$current->collection_id])}}" class="pull-right padr">Add more</a>
    </div>
    <div class="panel-body">
        ...
        @foreach ($items as $current)
            <div class="tab-pane{{language() != $current->language ? '' : ' active'}}" id="item-{{$current->language}}">
                {!! Form::model($current, [
                    'method'    => 'put',
                    'url'       => cms_route('companies.update', [$current->collection_id, $current->id], is_multilanguage() ? $current->language : null),
...
@include('admin.companies.scripts')
@endsection

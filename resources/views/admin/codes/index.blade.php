...
<div class="page-title">
    <div class="title-env">
        <h1 class="title">
            <i class="{{$iconCurrent = icon_type('coupons_codes')}}"></i>
            {{ $parent->type }}
        </h1>
                        ...
                        <li id="item{{ $item->id }}" class="item" data-id="{{ $item->id }}" data-pos="{{$item->position}}">
                            <div class="uk-nestable-item clearfix">
                                <div class="row">
                                    ...
                                    @endif
                                        <div class="list-label" @if($item->status == 1) style="background-color: green" @endif><a href="{{ $editUrl = cms_route('coupons_codes.edit', [$parent->id, $item->id]) }}">{{ $item->code }}</a></div>
                                    </div>
                                    <div class="col-sm-5 col-xs-2">
                                        <div class="btn-action togglable pull-right">
                                            ...
                                            {{--{!! Form::open(['method' => 'post', 'url' => cms_route('coupons_codes.visibility', [$item->id]), 'class' => 'visibility', 'id' => 'visibility' . $item->id]) !!}--}}
                                                ...
                                                {{--</button>--}}
                                            {{--{!! Form::close() !!}--}}
                                            {{--<a href="{{ cms_route('files.index', ['coupons_codes', $item->id]) }}" class="btn btn-{{$item->files_count ? 'turquoise' : 'white'}}" title="{{trans('general.files')}}">--}}
                                                {{--<span class="{{icon_type('files')}}"></span>--}}
                                            {{--</a>--}}
                                            {{--<a href="{{ $editUrl }}" class="btn btn-orange" title="{{trans('general.edit')}}">--}}
                                                {{--<span class="fa fa-edit"></span>--}}
                                            {{--</a>--}}
                                            {{--{!! Form::open(['method' => 'delete', 'url' => cms_route('coupons_codes.destroy', [$parent->id, $item->id]), 'class' => 'form-delete']) !!}--}}
...
@push('body.bottom')
@include('admin._scripts.move', ['route' => cms_route('coupons_codes.move', [$parent->id]), 'column' => 'collection_id', 'list' => $parentSimilar, 'parentId' => $parent->id])
<script type="text/javascript">
$(function() {
@if ($parent->admin_order_by == 'position')
    positionable('{{ cms_route('coupons_codes.updatePosition') }}', '{{$parent->admin_sort}}', '{{request('page', 1)}}', '{{$items->hasMorePages()}}');
@endif
});
</script>

...
<div class="panel panel-default">
    <div class="panel-heading">
        <h2 class="panel-title">Edit catalog</h2>
        ...
        <a href="{{cms_route('catalog.create', [$current->collection_id])}}" class="pull-right padr">Add more</a>
    </div>
    <div class="panel-body">
        ...
        @foreach ($items as $current)
            <div class="tab-pane{{language() != $current->language ? '' : ' active'}}" id="item-{{$current->language}}">
                {!! Form::model($current, [
                    'method'    => 'put',
                    'url'       => cms_route('catalog.update', [$current->collection_id, $current->id], is_multilanguage() ? $current->language : null),
